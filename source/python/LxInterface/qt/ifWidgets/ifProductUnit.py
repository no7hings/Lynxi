...
class IfSceneOverviewUnit(ifWidgetBasic.IfProductUnitOverviewUnitBasic):
        ...
        def setBranch(seq, key, value):
            def getPreviewLis(linkLis):
                ...
                for sceneLink in linkLis:
                    ...
                    tempPreviewFile = lxBasic.getOsTempVedioFile(lxBasic.toOsFileReplaceFileName(previewFile, viewFileName))
                    #
                    previewFileLis.append(
                        ((previewFile, tempPreviewFile), (lxBasic._toStringPrettify(sceneLink), 'Viewport ( {} )'.format(sceneVariant), (startFrame, endFrame)))
                    )
                #
    ...
    def setCameraRefresh(self):
        ...
        def setBranch(seq, key, value):
            ...
            previewFileLis = [(subPreviewFile, (lxBasic._toStringPrettify(scenePr.getSceneLink(sceneStage)), 'Viewport', (startFrame, endFrame)))]
            messageLis.append((4, previewFileLis))
            ...
            def getSubBuildData(data):
                for subIconKeyword, subAstCheckState, subAstEnable, subAstTimeTag in data:
                    if subAstEnable:
                        ...
                        rgba = [(255, 255, 64, 255), (63, 255, 127, 255)][exists]
                        if exists:
                            messageLis.append((5, (rgba, (subIconKeyword, subAstCheckState), lxBasic.translateRecordViewTime(subAstTimeTag))))
                        else:
                            ...
                            if asbRefDatas:
                                for seq, j in enumerate(asbRefDatas):
                                    ...
                                    if lxBasic.isOsExist(sourceExtraFile):
                                        sourceExtraData = lxBasic.readOsJson(sourceExtraFile)
                                        if sourceExtraData:
