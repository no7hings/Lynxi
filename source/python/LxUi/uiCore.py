...
_iconRoot = lxConfigure.BasicPath().iconRoot
...
def _toLxOsIconFile(iconKeyword, ext='.png'):
    ...
    if iconKeyword:
        ...
        iconKeyword = none
    #
    osFile = '{}/{}/{}{}'.format(_iconRoot, subLabel, iconKeyword, ext)
    #
    if isMayaIcon:
        if lxBasic.isOsExistsFile(osFile):
            return osFile
        else:
            return '{}/{}/{}{}'.format(_iconRoot, subLabel, 'default', ext)
    else:
        return osFile
...
# Tool Tip Box
class UiTooltipWidget(QWidget, _methodBasic.LxUiMethodBasic):
    def __init__(self, *args, **kwargs):
        self.clsSuper = super(UiTooltipWidget, self)
        self.clsSuper.__init__(*args, **kwargs)
        #
...
def getTooltipDelayTime():
    if lxConfigure.isTooltipAutoShow() is False:
...
def closeTooltipAutoShow():
    if UiTipTimer.isActive():
        lxConfigure.setTooltipAutoShow(False)
        UiTipTimer.stop()
        ...
        def show():
            if uiTip:
                if not hasattr(self, '_tooltipWidget'):
                    self._tooltipWidget = UiTooltipWidget(self)
                #
                self._tooltipWidget.setTooltip(uiTip)
                self._tooltipWidget.tooltipShow()
                #
                lxConfigure.setTooltipAutoShow(True)
            #
...
def setTreeWidgetKeywordFilter(treeWidget, filterWidget, filterLimitLis=None):
    if treeWidget and filterWidget:
        ...
        if hasattr(filterWidget, '_setUiPressStyle'):
            if not check:
                filterWidget._setUiPressStyle(ErrorStatus)
            else:
                filterWidget._setUiPressStyle(OnState)
