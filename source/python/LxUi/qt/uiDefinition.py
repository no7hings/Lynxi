...
# Widget
class UiWidgetDef(_methodBasic.LxUiMethodBasic):
    ...
    def setViewportLayout(self, widget):
        if hasattr(widget, '_viewportLayout'):
            ...
            self._viewportLayout = uiCore.QVBoxLayout(self._viewport)
        #
...
class UiExpandDef(UiWidgetDef):
    ...
    def _initExpandAbsUi(self):
        ...
        self._uiExpandIcon = uiCore._toLxOsIconFile(self._uiExpandIconKeyword)
        #
        self._uiUnexpandedSizePolicy = uiCore.QSizePolicy.Expanding, uiCore.QSizePolicy.Fixed
        self._uiExpandedSizePolicy = uiCore.QSizePolicy.Expanding, uiCore.QSizePolicy.Fixed
    ...
    # Override
    def _updateUiExpandStyle(self):
        if self.isExpandEnable():
            if self.isExpandable():
                self._setUiExpandStyle([uiCore.UnexpandState, uiCore.ExpandedState][self.isExpanded()])
            else:
                self._setUiExpandStyle(uiCore.UnexpandableState)
            #
            self._updateWidgetState()
    #
    def _setUiExpandStyle(self, state):
        if state is uiCore.UnexpandableState:
            ...
            elif state is uiCore.UnexpandState:
                self._uiExpandIconKeyword = 'svg_basic@svg#expandClose'
            #
            self._uiExpandIcon = uiCore._toLxOsIconFile(self._uiExpandIconKeyword + ['', 'on'][self.isExpandHovered()])
    #
    ...
    def isExpandPressRectContain(self, pos):
        return uiCore.isRectContainPos(self.expandPressRect(), pos)
    #
...
class UiPressDef(UiWidgetDef):
    ...
    def _initPressDefUi(self):
        ...
        #
        self._uiPressStatus = uiCore.NormalStatus
        #
        self._uiMargins = 0, 0, 0, 0
    #
        ...
        if self.isPressEnable():
            if self.isPressable():
                if self.isPressed():
                    ...
                    self._setUiPressStyle(uiCore.UnpressedState)
            else:
    ...
    def _setUiPressStyle(self, state):
        if state is uiCore.UnpressableState:
            ...
            if state is uiCore.PressedState:
                self.widget()._uiBackgroundRgba = 0, 0, 0, 0
                self.widget()._uiBorderRgba = 0, 0, 0, 0
                #
                self.widget()._uiNameRgba = 255, 255, 255, 255
            elif state is uiCore.UnpressedState:
                self.widget()._uiBackgroundRgba = 0, 0, 0, 0
                self.widget()._uiBorderRgba = 0, 0, 0, 0
                #
...
#
class UiGraphUnitDef(UiWidgetDef):
...
class UiViewDef(UiWidgetDef):
    ...
    def _initViewDefUi(self):
        ...
        self._uiPlaceholderImage = uiCore._toLxOsIconFile('svg_basic@svg#empty')
    #
...
#
class UiTabViewDef(UiWidgetDef):
    ...
    #
    def setTabPosition(self, value):
        if value in [uiCore.South, uiCore.North]:
            w, h = self.tabSize()
            self.setTabSize(w, h)
        else:
            w, h = self.tabSize()
...
class UiWindowDef(UiWidgetDef):
    ...
    def _initWindowDefAttr(self):
        ...
        #
        self._direction = uiCore.Horizontal
        #
    ...
    def _initWindowDefAction(self):
        self._progressThread = uiCore.QThread_()
    #
...
#
class UiScrollBarDef(UiWidgetDef):
...
#
class UiScrollAreaDef(UiWidgetDef):
    ...
    def setScrollBar(self, widget):
        if hasattr(widget, '_hScrollBar'):
            ...
            #
            self.hScrollBar().viewModel().setDirection(uiCore.Horizontal)
        ...
        if hasattr(widget, '_vScrollBar'):
            ...
            #
            self.vScrollBar().viewModel().setDirection(uiCore.Vertical)
...
#
class UiSplitterDef(UiWidgetDef):
        ...
        if self.isPressEnable():
            if self.isPressable():
                if self.isPressCurrent():
                    self._setUiPressStyle(uiCore.CurrentState)
                else:
...
class UiGroupModelDef(
    ...
    def addWidget(self, widget):
        ...
        if verticalSizePolicy == uiCore.QSizePolicy.Expanding:
            self.setExpandedSizePolicy(uiCore.QSizePolicy.Expanding, uiCore.QSizePolicy.Expanding)
            #
...
#
class UiSplitterModelDef(
    UiSplitterDef
):
    def _initUiSplitterModelDef(self):
