...
#
class UiRadarChartModel(uiModelBasic.UiChartModelBasic):
    ...
    def _updateDrawDatum(self):
        ...
        if datum:
            ...
            spacing = 8
            #
            if self._hAlign is uiCore.AlignLeft:
                xOffset = xPos + side
            elif self._hAlign is uiCore.AlignHCenter:
                xOffset = xPos + (width - radius)/2
            else:
                xOffset = width - radius - side
            if self._vAlign is uiCore.AlignTop:
                yOffset = yPos + side
            elif self._vAlign is uiCore.AlignVCenter:
    ...
    def _toRadarMapDrawDatum(cls, drawDatum, xOffset, yOffset, radius):
        ...
        #
        backgroundGradient = uiCore.QtGui.QConicalGradient(cx, cy, ia)
        for seq, rgba in enumerate(rgbaLis0):
            r, g, b, a = rgba
            backgroundGradient.setColorAt(float(seq) / float(len(rgbaLis0)), uiCore.QtGui.QColor(r, g, b, 127))
        #
        r, g, b, a = rgbaLis0[0]
        backgroundGradient.setColorAt(1, uiCore.QtGui.QColor(r, g, b, 127))
        #
...
#
class UiSectorChartModel(uiModelBasic.UiChartModelBasic):
