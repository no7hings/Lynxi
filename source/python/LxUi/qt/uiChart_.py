...
def getPieDrawData(data, width, height, side):
    def getBranch(inData, seq=0, qa=90, ma=0):
        ...
        #
        percent = '%s%%' % (uiCore.toShowPercent(maxValue, value, roundCount=4))
        #
...
#
class xPieChart(uiCore.QWidget_):
    def __init__(self, *args, **kwargs):
        self.clsSuper = super(xPieChart, self)
        ...
        self.initUi()
        #
        self.setUiSize(self._uiWidth, self._uiHeight)
    #
    ...
    def paintEvent(self, event):
        ...
        if self._pieDrawData:
            ...
            if self._subExplain is not None:
                ...
                painter.setFont(_font(size=10, weight=50, family=uiCore._families[1]))
        ...
        if self._selPathData:
            for seq, i in enumerate(self._selPathData):
                ...
                if path.contains(event.pos()) is True:
                    if isPress is False:
                        isPress = True
                        self._subExplain = '{0} : {1}'.format(self._uiNameText, explain)
...
#
class xMapper(uiCore.QWidget_):
    width = 380
    xPosition = 20
    yPosition = 0
    def __init__(self, *args, **kwargs):
        self.xMapperSuper = super(xMapper, self)
        self.xMapperSuper.__init__(*args, **kwargs)
        #
    ...
    #         self.xMapperSuper.mousePressEvent(event)
...
#
class xSequenceChart(uiCore.QWidget_):
    def __init__(self, *args, **kwargs):
        self.clsSuper = super(xSequenceChart, self)
        self.clsSuper.__init__(*args, **kwargs)
        #
        self.setAttribute(QtCore.Qt.WA_TranslucentBackground)
        #
            ...
            if self._rangeArray is not None:
                ...
                for numRange in rangeArray:
                    if isinstance(numRange, int):
                        ...
                        r, g, b = uiCore.hsvToRgb(140, 1 * percent, 1)
...
#
class xHistogramChart(uiCore.QWidget):
    def __init__(self, *args, **kwargs):
        ...
        if self._valueArray:
            ...
            if maxValue:
                ...
                for seq, v in enumerate(self._valueArray):
                    ...
                    r, g, b = uiCore.hsvToRgb(140 * rgbPercent, 1, 1)
                    #
                    painter.setBackgroundRgba(r, g, b, 255)
                    painter.setBorderRgba(r, g, b, 255)
                    #
                ...
                if currentValueX is not None and currentValueY is not None:
                    ...
                    #
                    painter.setBorderRgba(223, 223, 223, 255)
                    painter.setFont(_font(size=12, weight=75))
                    painter.drawText(
                        textRect,
                        QtCore.Qt.AlignLeft | QtCore.Qt.AlignTop,
                        '{2} ( {0} )\r\n{3} ( {1} )'.format(self._xValueExplain, self._yValueExplain, currentValueX, lxBasic.getShowNumber(currentValueY, useMode=self._useMode))
                    )
        #
        painter.setDrawAxis(
...
#
class xColorChart(uiCore.QWidget):
    def __init__(self, *args, **kwargs):
        self.clsSuper = super(uiCore.QWidget, self)
        self.clsSuper.__init__(*args, **kwargs)
        #
    ...
    def paintEvent(self, event):
        def setDrawColor():
            def setDrawBranch(x, y):
                ...
                if not p in points:
                    ...
                    if mainColorPath.contains(colorPoint):
                        ...
                        colorPath = uiCore.QPainterPath_()
            ...
            mainColorPath = uiCore.QPainterPath_()
    ...
    def zoomAction(self, delta):
        radix = 3
        #
        self._count = lxBasic.mapStepValue(self._count, -delta, radix, self._countMaximum, self._countMinimum)
        #
        self.update()
    #
    ...
    def trackAction(self, point):
        ...
        xRadix = 5.0
        yRadix = 5.0
        self._vMult = lxBasic.mapStepValue(self._vMult, -yDelta, yRadix, self._vMultMaximum, self._vMultMinimum)
        #
        self.update()
    #
