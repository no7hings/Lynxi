...
#
from LxUi.qt import uiAbstract, uiAction
#
QtGui = uiCore.QtGui
...
# Basic Item
class _UiItemModelBasic(
    uiAbstract.UiItemModelAbs,
    uiAbstract.UiDatumEnterItemModelAbs
):
...
# Basic View
class _UiViewModelBasic(
    uiAbstract.UiViewModelAbs,
    uiAbstract.UiScrollAreaModelAbs
):
    ...
    def _initViewModelBasicAction(self):
        self._trackActionModel = uiAction.UiTrackActionModel(self)
        self._trackActionModel.setMinimumPos(0, 0)
    #
    def _itemSize(self):
        if self._itemMode == uiCore.ListMode:
            w, h = self._viewportWidth - self._vScrollWidth(), self._uiItemHeight
        elif self._itemMode == uiCore.IconMode:
        ...
        if self.visibleIndexCount() > 0:
            for itemIndex in self.visibleItemIndexes():
                ...
                if itemIndex in self._itemModelVisiblePosDic:
                    ...
                    widget.hide()
            #
            if self.itemMode() is uiCore.TreeMode:
                self._updateWidget()
    #
    ...
    def _hoverExecuteAction(self, event):
        ...
        # Tree Connection
        if self._itemMode is uiCore.TreeMode:
...
#
class _UiScrollAreaModelBasic(uiAbstract.UiScrollAreaModelAbs):
...
#
class _UiItemModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)


#
class _UiViewModel(_UiViewModelBasic):
    def __init__(self, widget):
        self._initViewModelBasic(widget)


#
class UiTreeItemModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)


#
class UiValueEnterItemModel(uiAbstract.UiValueEnterItemModelAbs):
    def __init__(self, widget):
...
# Filter Item
class UiFilterEnterItemModel(uiAbstract.UiDatumEnterItemModelAbs):
    def __init__(self, widget):
        self._initDatumEnterItemModelAbs()
        #
        self.__overrideAttr()
...
#
class UiDatumEnterItemModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)
        #
        self.__overrideAttr()
    ...
    def _updateUiEnterState(self):
        ...
        if self.isEnterable():
            if self.isEntered():
                self.setUiEnterState(uiCore.EnterState)
            else:
                self.setUiEnterState(uiCore.UnenterState)
        else:
            self.setUiEnterState(uiCore.NormalState)
        #
        self._updateWidgetState()
    #
    ...
    def _entryAction(self):
        ...
        #
        self._updateUiPressStatusByDatum()
    #
    ...
    def setUiEnterState(self, state):
        if state is uiCore.NormalState:
            ...
            if state is uiCore.EnterState:
                self.widget()._uiEnterBorderRgba = 63, 127, 255, 255
            elif state is uiCore.UnenterState:
...
# Text Brower
class UiTextBrowerItemModel(uiAbstract.UiDatumEnterItemModelAbs):
    def __init__(self, widget):
        self._initDatumEnterItemModelAbs()
        #
        self.__overrideAttr()
...
# Scroll Bar
class UiScrollBarModel(uiAbstract.UiScrollBarModelAbs):
    def __init__(self, widget):
...
#
class _UiTabModelBasic(uiAbstract.UiTabItemModelAbs):
    ...
    def _updateRectGeometry(self):
        ...
        frameWidth, frameHeight = self.frameSize()
        #
        if self.tabPosition() == uiCore.South or self.tabPosition() == uiCore.North:
            w, h = self.itemSize()
        else:
            h, w = self.itemSize()
        #
        ...
        if self.tabPosition() == uiCore.West:
            # Icon
            if self.icon() is not None:
                self.iconRect().setRect(
                    w - h - side + _w, yPos + _h,
    ...
    def _setUiPressStyle(self, state):
        if state is uiCore.UnpressableState:
            ...
            if state is uiCore.CurrentState:
            ...
            elif state is uiCore.NormalState:
...
#
class UiTabModel(_UiTabModelBasic):
    def __init__(self, widget):
        self._initTabModelBasic(widget)


#
class UiButtonTabModel(_UiTabModelBasic):
    def __init__(self, widget):
        self._initTabModelBasic(widget)
    #
...
# Tab Item
class UiShelfTabModel(_UiTabModelBasic):
    def __init__(self, widget):
        self._initTabModelBasic(widget)
    #
    def _updateRectGeometry(self):
        ...
        iconWidth, iconHeight = self.iconSize()
        #
        if self.tabPosition() is uiCore.South or self.tabPosition() is uiCore.North:
            if not self.isPressCurrent() and not self.isPressHovered():
                yPos += 4
                h -= 4
                # noinspection PyArgumentEqualDefault
                self.widget().setFont(uiCore.xFont(size=8, weight=50, family=uiCore._families[2]))
            else:
                # noinspection PyArgumentEqualDefault
                self.widget().setFont(uiCore.xFont(size=10, weight=50, family=uiCore._families[2]))
            #
            self.nameTextRect().setRect(
                xPos, yPos,
                w, h
...
# Choose Tab Item
class UiChooseTabModel(uiAbstract.UiDatumEnterItemModelAbs):
    def __init__(self, widget):
        self._initDatumEnterItemModelAbs()
        #
        self.setWidget(widget)
...
#
class _UiTabBarModelBasic(uiAbstract.UiTabBarModelAbs):
...
# Tab Bar
class UiTabBarModel(_UiTabBarModelBasic):
    def __init__(self, widget):
        self._initTabBarModelBasic(widget)


#
class _UiTabGroupModelBasic(uiAbstract.UiTabViewModelAbs):
    ...
    def _updateViewportGeometry(self):
        ...
        #
        if self.tabPosition() is uiCore.West:
            _h = (w - buttonHeight) / 2
            # Tab Bar
        ...
        elif self.tabPosition() is uiCore.East:
            _h = (w - buttonHeight) / 2
            # Tab Bar
            self.tabBar().setGeometry(
                width - w, yPos,
        ...
        elif self.tabPosition() is uiCore.South:
            _w = (h - buttonWidth) / 2
...
# Tab View
class UiTabGroupModel(_UiTabGroupModelBasic):
    def __init__(self, widget):
        self._initTabViewModelBasic(widget)


# Window
class UiWindowModel(uiAbstract.UiWindowModelAbs):
    def __init__(self, widget):
    ...
    def _initWindowBasic(self):
        ...
        self._initWindowBasicRect()
        self._initWindowBasicAction()
        self._initWindowBasicVar()
    #
    ...
    def _initWindowBasicUi(self):
        pass
    #
    def _initWindowBasicRect(self):
        pass
    #
    def _initWindowBasicAction(self):
        self._progressTimer.timeout.connect(self.setProgressZero)
    #
    def _initWindowBasicVar(self):
        pass
    #
    ...
    def _updateRectGeometry(self):
        ...
        #
        if self.direction() is uiCore.Horizontal:
    ...
    def _updateMenuButtonGeometry(self):
        ...
        buttonWidth, buttonHeight = self._uiButtonWidth, self._uiButtonHeight
        #
        isHorizontal = self.direction() is uiCore.Horizontal
        #
    ...
    def _updateMenuRectGeometry(self):
        ...
        if self.isMenuEnable():
            ...
            if self.direction() == uiCore.Horizontal:
                x, y = xPos, yPos
    ...
    def _resizeAction(self, event, message):
        if self.isResizeable():
            return uiCore.nativeEve(self._widget, message)
        else:
            return False, 0
    # Button
...
# Action Item
class UiActionItemModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)
        #
        self.__overrideAttr()
    ...
    def _updateUiStyle(self):
        if (self.isPressable() or self.isExtendEnable()) and not self.isSeparator():
            self._setUiPressStyle([uiCore.UnpressedState, uiCore.PressedState][self.isPressCurrent()])
        else:
            self._setUiPressStyle(uiCore.UnpressableState)
        #
        if self._isCheckEnable is True:
            if self._isCheckable is True:
                self._setUiCheckStyle([uiCore.UncheckedState, uiCore.CheckedState][self.isChecked()])
            else:
                self._setUiCheckStyle(uiCore.UncheckableState)
    #
    def _setUiPressStyle(self, state):
        if state is uiCore.UnpressableState:
            if self._uiIconKeyword is not None:
                self._uiIcon = uiCore._toLxOsIconFile('svg_basic@svg#unused')
            ...
            if state is uiCore.UnpressedState:
                if self._uiIconKeyword is not None:
                    self._uiIcon = uiCore._toLxOsIconFile(self._uiIconKeyword)
                if self._uiExtendIconKeyword is not None:
                    self._uiExtendIcon = uiCore._toLxOsIconFile(self._uiExtendIconKeyword)
                #
                self.widget()._uiBackgroundRgba = 0, 0, 0, 0
                self.widget()._uiBorderRgba = 0, 0, 0, 0
                #
                ...
                #
                self.widget()._uiFontItalic = False
            elif state is uiCore.PressedState:
                if self._uiIconKeyword is not None:
                    self._uiIcon = uiCore._toLxOsIconFile(self._uiIconKeyword + 'on')
                if self._uiExtendIconKeyword is not None:
                    self._uiExtendIcon = uiCore._toLxOsIconFile(self._uiExtendIconKeyword + 'on')
                #
                self.widget()._uiBackgroundRgba = 71, 71, 71, 255
            ...
            #
            if self._uiSubIconKeyword is not None:
                self._uiSubIcon = uiCore._toLxOsIconFile(self._uiSubIconKeyword)
        #
        self._updateWidgetState()
    #
    def update(self):
...
# Action Drop View
class UiActionDropViewModel(uiAbstract.UiActionDropViewModelAbs):
...
# Choose View
class UiChooseViewModel(_UiViewModelBasic):
    def __init__(self, widget):
        self._initViewModelBasic(widget)
        #
...
# Choose Drop View
class UiChooseDropViewModel(uiAbstract.UiChooseDropViewModelAbs):
    def __init__(self, widget, itemClass):
...
# Icon Button
class UiIconButtonModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)
        #
        self.__overrideAttr()
    ...
    def _setUiPressStyle(self, state):
        if state is uiCore.UnpressableState:
            if self._uiIconKeyword is not None:
                self._uiIcon = uiCore._toLxOsIconFile(self._uiIconKeyword + 'off')
            #
            self.widget().update()
            #
            self.widget()._uiNameRgba = 95, 95, 95, 255
            ...
            if state is uiCore.NormalState:
                if self._uiIconKeyword is not None:
                    self._uiIcon = uiCore._toLxOsIconFile(self._uiIconKeyword + ['', 'on'][self.isPressHovered()])
                #
                if self._uiExtendIconKeyword is not None:
                    self._uiExtendIcon = uiCore._toLxOsIconFile(self._uiExtendIconKeyword + ['', 'on'][self.isExtendPressHovered()])
                #
                self.widget()._uiFontItalic = False


# Press Button
class UiPressbuttonModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)
        #
        self.__overrideAttr()
        ...
        if self.isPercentEnable():
            ...
            if status is uiCore.OffStatus:
                ...
                if status is uiCore.NormalStatus:
                    ...
                    if status is uiCore.ErrorStatus:
                        r, g, b = 255, 0, 64
    ...
    def _updateUiPressStyle(self):
        if self.isPressEnable():
            if self.isPressable():
                if self._pressFlag is True:
                    self._setUiPressStyle(uiCore.PressedState)
                else:
                    self._setUiPressStyle(uiCore.NormalState)
                    self._updateUiPressStatus()
            else:
...
# Attribute Item
class UiAttributeItemModel(_UiItemModelBasic):
    def __init__(self, widget):
        self._initItemModelBasic(widget)
        #
        self.__overrideAttr()
...
#
class UiChartModelBasic(uiAbstract.UiChartModelAbs):
    _angle = math.radians
    _sin = math.sin
    _cos = math.cos
    _tan = math.tan
...
#
class UiGroupModelBasic(uiAbstract.UiGroupModelAbs):
    ...
    def setUnpin(self):
        ...
        if hasattr(self, '_viewportGeometryRecord'):
            ...
            w = self.width() + 8
            h = uiCore.getWidgetMinimumHeight(self.viewport()) + 32 + 8
            op = self.widget().pos()
    ...
    def setViewport(self, widget):
        if hasattr(widget, '_viewport'):
            ...
            self._viewport = uiCore.QWidget_(self.widget())
        #
        self._widgetLayout = uiCore.QVBoxLayout(self.widget())
...
#
class UiGroupModel(UiGroupModelBasic):
    def __init__(self, widget):
