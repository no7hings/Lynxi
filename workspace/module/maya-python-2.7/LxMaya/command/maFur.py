...
def getYetiGuideHairSystems(yetiObject):
    lis = []
    yetiNode = maUtils._dcc_getNodShapeNodepathStr(yetiObject)
            ...
            if maUtils._isAppExist(guideSet):
                ...
                for curve in curves:
                    if maUtils._isAppExist(curve):
                        curvePath = maUtils._dcc_getNodFullpathNodepathStr(curve)
                        curveShape = maUtils._dcc_getNodShapeNodepathStr(curvePath, 1)
                        follicles = maUtils.getInputObjectsByAttrName(curveShape, '.create')
                        if follicles:
                            follicle = follicles[0]
                            follicleShape = maUtils._dcc_getNodShapeNodepathStr(follicle, 1)
                            #
...
def setYetiObjectsWriteCache(yetiObjects, yetiCaches, startFrame, endFrame, sample=3, isUpdateViewport=True, isGeneratePreview=True):
    ...
    yetiShapePathLis = [maUtils._dcc_getNodShapeNodepathStr(i) for i in yetiObjects]
...
def getNurbsHairMapNodes(nurbsHairObject):
    ...
    #
    lis = []
    shapePath = maUtils._dcc_getNodShapeNodepathStr(nurbsHairObject)
...
def getYetiGraphData(yetiObject):
    yetiNode = maUtils._dcc_getNodShapeNodepathStr(yetiObject, fullPath=1)
...
def setYetiObjectCloseSolver(yetiObject):
    ...
    if hairSystemObjects:
        for hairSystemObject in hairSystemObjects:
            hairSystem = maUtils._dcc_getNodShapeNodepathStr(hairSystemObject, 1)
            #
...
def setSolverGroup(assetName, namespace=none):
    ...
    [maUtils.setHide(maUtils._dcc_getNodTransformNodepathStr(i)) for i in forHides]
                ...
                if attrData:
                    ...
                    if maUtils._isAppExist(sourceObject):
                        ...
                        if not maUtils._isAppExist(blendShapeNode):
                            ...
                            inOrig = growSourceObject + '|' + maUtils._dcc_getNodShapeNodepathStr(growSourceObject, 1).split(':')[-1] + origLabel
                            #
                            ...
                            sourceShapePath = maUtils._dcc_getNodShapeNodepathStr(sourceObject)
                            ...
                            #
                            growSourceShapePath = maUtils._dcc_getNodShapeNodepathStr(growSourceObject)
...
def setScAstSolverGuideConnectToCfx(connectionDic, scAstCfxNamespace, scAstSolverNamespace):
    def setBranch(nodepathString, connectionLis):
        ...
        if maUtils._isAppExist(scObjectName):
            scObjectShapePath = maUtils._dcc_getNodShapeNodepathStr(scObjectName)
...
def setScAstSolverCurveConnectToSolverCache(assetName, scAstSolverNamespace, scAstSolverCacheNamespace):
    def setBranch(nodepathString):
        ...
        if maUtils._isAppExist(sourceObjectName):
            sourceShapePath = maUtils._dcc_getNodShapeNodepathStr(sourceObjectName)
        ...
        for growSourceObject in growSourceObjects:
            if growSourceObject.endswith(astCfxGrowSourceGroupLabel):
                ...
                if maUtils._isAppExist(sourceObject):
                    ...
                    #
                    nodeLis.append(inShapeNode)
                    inOrig = maUtils._dcc_getNodShapeNodepathStr(growSourceObject, 1) + origLabel
                    #
...
def setScAstNurbsHairSolverConnectToTime(assetName, astCfxNamespace, startFrame):
    def setConnectToTime(objectPath):
        shapePath = maUtils._dcc_getNodFullpathNodepathStr(objectPath)
        #
...
def getYetiSolverMode(yetiObject):
    shapePath = maUtils._dcc_getNodShapeNodepathStr(yetiObject, 1)
...
def getHairSystemSolverMode(hairSystemObject):
    shapePath = maUtils._dcc_getNodShapeNodepathStr(hairSystemObject, 1)
...
def getNurbsHairSolverMode(nurbsHairSolverObject):
    shapePath = maUtils._dcc_getNodShapeNodepathStr(nurbsHairSolverObject, 1)
...
def getNurbsHairCacheFile(nurbsHairObject):
    shapePath = maUtils._dcc_getNodShapeNodepathStr(nurbsHairObject, 1)
...
def getPfxHairSimMode(pfxHairObject):
    pfxHairNode = maUtils._dcc_getNodShapeNodepathStr(pfxHairObject)
...
def getFurObjectsPathDic(furObjects):
    ...
    if furObjects:
        for nodepathString in furObjects:
            objectPath = maUtils._dcc_getNodFullpathNodepathStr(nodepathString)
...
def setRefreshYetiGrow(growMesh):
    meshShape = maUtils._dcc_getNodShapeNodepathStr(growMesh)
...
def getNurbsHairConnectObjectData(nurbsHairObject):
    def getBranch(nodepathString):
        if maUtils._getNodeIsTransform(nodepathString):
            ...
            #
            nodepathString = maUtils._dcc_getNodShapeNodepathStr(objectPath)
        else:
        ...
        if growMeshes:
            [graphGrowGeometryLis.append(maUtils._dcc_getNodFullpathNodepathStr(i)) for i in growMeshes if i not in graphGrowGeometryLis]
        if guideMeshes:
            [graphGuideGeometryLis.append(maUtils._dcc_getNodFullpathNodepathStr(i)) for i in guideMeshes if i not in graphGuideGeometryLis]
    #
...
def getNurbsHairObjects(used=False):
    data = [maUtils._dcc_getNodTransformNodepathStr(i, 1) for i in maUtils.getNodeLisByType('nurbsHair', fullPath=True)]
    if used is True:
        subLis = []
        for i in data:
...
def getNurbsHairGraphNodes(nurbsHairObject):
    def getBranch(mNode):
        if maUtils._getNodeIsTransform(mNode):
            objectPath = maUtils._dcc_getNodFullpathNodepathStr(mNode)
            mNode = maUtils._dcc_getNodShapeNodepathStr(objectPath)
            #
...
def getNhrObjectGraphGeometryObjects(nurbsHairObject):
    def getBranch(mNode):
        ...
        if geometryObjects:
            [lis.append(maUtils._dcc_getNodFullpathNodepathStr(i)) for i in geometryObjects if i not in lis]
    #
    lis = []
    getBranch(nurbsHairObject)
    return lis
...
def setConnectNurbsHairSolver(nurbsHairSolverObject, guideGroup):
    ...
    shapePath = maUtils._dcc_getNodShapeNodepathStr(nurbsHairSolverObject)
...
def getNhrSolverGuideCacheObjects(mhrSolverGuideObjects):
    def getBranch(nodepathString):
        maShape = maUtils._dcc_getNodShapeNodepathStr(nodepathString)
...
def setNurbsHairCovertToCache(nurbsHairObject, cacheFolder, timeTag=None):
    ...
    #
    nurbsHairShape = maUtils._dcc_getNodShapeNodepathStr(nurbsHairObject)
...
def setNurbsHairObjectCreateSolverGuide(nurbsHairObject, assetName):
    ...
    if not boolean:
        ...
        if nhrScatterObjects:
            ...
            nhrScatterShape = maUtils._dcc_getNodShapeNodepathStr(nhrScatterObject)
