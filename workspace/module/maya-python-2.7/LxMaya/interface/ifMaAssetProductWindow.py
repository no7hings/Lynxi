...
_version = shmOutput.Scheme().version


#
class IfAssetProductToolWindow(guiQtWidgets.QtToolWindow):
    widthSet = 400
    def __init__(self, parent=qtCore.getAppWindow()):
        super(IfAssetProductToolWindow, self).__init__(parent)

        self.setNameString(_title)
        self.setIndexString(_version)
        #
    ...
    def setAssetCheckList(self):
        ...
        if checkConfig:
            for k, v in checkConfig.items():
                ...
                if enable is True:
                    inspectionItem.setItemIcon(0, 'svg_basic/check')
                else:
                    inspectionItem.setItemIcon(0, 'svg_basic/checkdisable')
    #
    ...
    def setViewAstModelCheckResult(self):
        ...
        for seq, (k, v) in enumerate(checkConfig.items()):
            ...
            if k in inData:
                ...
                if errorData:
                    isError = False
                    inspectionItem.setItemCheckIcon(0, 'svg_basic/check', 'warning')
                    for i in inCheck:
    ...
    def setupToolUnits(self):
        self._kit__unit__set_left_build_(self.leftToolScrollBox, self.leftBottomToolBar)
    ...
    def setAstTopToolBar(self, layout):
        ...
        #
        self._infoLabel = guiQtWidgets.QtValueLine()
        layout.addWidget(self._infoLabel)
        self._infoLabel.setNameTextWidth(0)
        #
        self.filterEnterLabel = guiQtWidgets.QtFilterLine()
        layout.addWidget(self.filterEnterLabel)
        self.treeBox.setKeywordFilterWidgetConnect(self.filterEnterLabel)
        #
    ...
    # Tool Panel
    def _kit__unit__set_left_build_(self, toolBoxLayout, toolBarLayout):
        ...
        for i in uiDatumLis:
            ...
            title = toolUnit.VAR_kit__qt_wgt__unit__uiname
            iconKeywordStr = toolUnit.VAR_kit__qt_wgt__unit__icon
            tooltip = toolUnit.VAR_kit__qt_wgt__unit__tip
            # Toggle Button
            toggleButton = None
            ...
            toggleButton.setIconExplain(iconKeywordStr, 32, 32)
            #
    ...
    def setupRightWidget(self, toolBarLayout):
        ...
        for i in uiDatumLis:
            key, iconKeywordStr, tooltip, visible, connectMethodLis = i
            toggleButton = None
            toggleButtonCreateCmd = 'self.{0}ToggleButton = qtWidgets_.QRadioButton_();toggleButton = self.{0}ToggleButton'.format(key)
            exec toggleButtonCreateCmd
            toggleButton.setIconExplain(iconKeywordStr, 32, 32)
    ...
    def setupWindow(self):
        ...
        #
        self.rightToolboxGroup = guiQtWidgets.QtToolboxGroup()
