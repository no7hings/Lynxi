...
from LxMaBasic import maBscCfg, maBscMethods
...
# File Label
astLayoutRigFileLabel = prsOutputs.Util.astLayoutRigFileLabel
astAnimationRigFileLabel = prsOutputs.Util.astAnimationRigFileLabel
astSimulationRigFileLabel = prsOutputs.Util.astSimulationRigFileLabel
#
none = ''
...
class IfScRigLoadedUnit(kitQtWgtAbs.AbsKitQtWgtUnit_):
    ...
    def _kit__unit__set_central_build_(self, layout):
        def setActions():
            ...
            actionDatumLis = [
                ('Basic', ),
                ('Load Asset(s) - Rig', ('svg_basic/file', 'svg_basic/load_action'), isLoadAstRigEnable, loadAstRigCmd)
            ]
    ...
    def setCentralRefresh(self):
        def setBranch(seq, key, value):
            def setSubActions():
                ...
                subActionData = [
                    ('Basic',),
                    ('Load Current Rig', ('svg_basic/file', 'svg_basic/load_action'), True, rigLoadCmd),
                    ('Extend',),
                    ('Open Current Rig Folder', 'svg_basic/folder', True, rigFolderOpenCmd)
                ]
            ...
            gridItem.setIcon('svg_basic/rig')
            #
            ...
            messages = [
                ...
                (messageWidget.DrawColor, (rgba, iconKeywordStr, bscMethods.OsTimestamp.toChnPrettify(mtimestamp)))
            ]
...
#
class IfScLayoutToolUnit(kitQtWgtAbs.IfToolUnitBasic):
    widthSet = 400
    #
    projectName = currentProjectName
    #
    VAR_kit__qt_wgt__unit__uiname = 'Layout Unit'
    #
    ...
    dicScCamTool = {
        ...
        'addCamera': [1, 2, 0, 1, 2, 'Add Camera [ 000 ]', 'svg_basic/add'], 'removeCamera': [1, 2, 2, 1, 2, 'Remove Camera [ 000 ]', 'svg_basic/remove'],
        3: 'Modify',
        'createCamera': [1, 4, 0, 1, 2, 'Create Default Camera', 'svg_basic/camera'], 'setCameraView': [1, 4, 2, 1, 2, 'Set Camera View', 'svg_basic/modify'],
        5: 'HUD',
        'openHud': [1, 6, 0, 1, 2, 'Open HUD', 'svg_basic/buttonOpen'], 'closeHud': [1, 6, 2, 1, 2, 'Close HUD', 'svg_basic/buttonClose'],
        7: 'Update',
        'updateCamera': [1, 8, 0, 1, 4, 'Update Camera(s) [ 000 ]', 'svg_basic/update']
    }
    #
    dicScPrvTool = {
        ...
        'updatePreview': [1, 6, 0, 1, 4, 'Update Preview [ 000 ]', 'svg_basic/camera']
    }
    # Import
    dicScCamImportTool = {
    ...
    def setupModifyTab(self, layout):
        # Camera Box
        self._cameraToolBox = guiQtWidgets.QtToolbox()
        ...
        # Preview Box
        self._previewToolBox = guiQtWidgets.QtToolbox()
    ...
    def setupCameraToolUiBox(self, toolBox):
        ...
        #
        self._scCameraFilterLabel = guiQtWidgets.QtFilterLine()
        ...
        self._availableCameraTreeViewBox.setKeywordFilterWidgetConnect(self._scCameraFilterLabel)
        #
        ...
        self._activeCameraTreeViewBox.setKeywordFilterWidgetConnect(self._scCameraFilterLabel)
        #
        self._addCameraButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self._removeCameraButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self._setCameraViewButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.updateCameraButton = guiQtWidgets.QtPressbutton()
    ...
    def setupPreviewToolUiBox(self, toolBox):
        ...
        #
        self._previewPercentLabel = guiQtWidgets.QtValueArrayLine()
    ...
    def setupImportTab(self, layout):
        self._importCameraBox = guiQtWidgets.QtToolbox()
    ...
    def setupCameraImportToolUiBox(self, toolBox):
        ...
        #
        self._scClassLabel = guiQtWidgets.QtValueLine()
        ...
        #
        self._scNameLabel = guiQtWidgets.QtValueLine()
        ...
        #
        self._scVariantLabel = guiQtWidgets.QtValueLine()
        ...
        #
        self._useMayaFileButton = guiQtWidgets.QtRadioCheckbutton()
    ...
    def setListScCamera(self):
        ...
        #
        defCameras = maBscCfg.MaUtility.DEF_mya_default_camera_list
        ...
        if availableCameraLis:
            for cameraPath in availableCameraLis:
                if not cameraPath in activeCameraLis:
                    cameraName = maUtils._nodeString2nodename_(cameraPath)
                    #
    ...
    def setModifyBtnState(self):
        ...
        self._addCameraButton.setNameString('Add Camera [ %s ]' % str(len(availableCameraLis)).zfill(3))
        #
        ...
        self._removeCameraButton.setNameString('Remove Camera [ %s ]' % str(len(activeCameraLis)).zfill(3))
        #
        self._setCameraViewButton.setPressable([False, True][subBoolean])
        #
        self.updateCameraButton.setPressable([False, True][subBoolean])
        self.updateCameraButton.setNameString('Update Camera(s) [ %s ]' % str(len(activeCameraLis)).zfill(3))
        #
        self._updateScenePreviewButton.setPressable([False, True][subBoolean])
        self._updateScenePreviewButton.setNameString('Update Preview [ %s ]' % str(len(activeCameraLis)).zfill(3))
        #
        self.setSelSwitch()
    #
    ...
    def setCreateCamera(self):
        if self._connectObject:
            ...
            if maUtils._isAppExist(sceneCamera):
            ...
            #
            sceneOp.setAddSceneCameras(sceneName, [maUtils._dcc_getNodFullpathNodepathStr(sceneCamera)])
    #
    ...
    def setCameraView(self):
        ...
        if cameraPaths:
            ...
            if maUtils._isAppExist(usedCamera):
        ...
        if self._connectObject:
            ...
            if usedCameras:
                ...
                maScUploadCmds.scUnitCamerasUploadCmd(
                    ...
                    startFrame, endFrame, prsOutputs.Util.animKeyFrameOffset,
                    timeTag,
                    withCamera
                )
                bscObjects.MessageWindow(
    ...
    def _updateScPreviewCmd(self):
        if self._connectObject:
            ...
            #
            vedioFormat = [prsOutputs.Util.aviExt, prsOutputs.Util.movExt][self.movFormatButton.isChecked()]
    ...
    def setImportCameraCmd(self):
        if self._connectObject:
            ...
            if bscMethods.OsFile.isExist(serverCameraFile):
                ...
                if not maUtils._isAppExist(cameraLocator):
                    maBscMethods.File.importFrom(serverCameraFile)
                    bscObjects.MessageWindow(
    ...
    def _kit__unit__set_build_(self):
        ...
        #
        widget = qtCore.QWidget_()
        self._tabWidget.addTab(widget, 'Import', 'svg_basic/tab')
        layout = qtCore.QVBoxLayout_(widget)
        layout.setAlignment(qtCore.QtCore.Qt.AlignTop)
...
#
class IfScAnimationLinkToolUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    # Utilities Tool
    dicUtilsTool = {
        'rigLoaded': [0, 0, 0, 1, 4, 'Asset ( Rig ) Loaded', 'svg_basic/subwindow'],
        #
        'animationManager': [0, 2, 0, 1, 2, 'Scene ( Animation ) Manager', 'svg_basic/subwindow'],  'simulationManager': [0, 2, 2, 1, 2, 'Scene ( Simulation ) Manager', 'svg_basic/subwindow']
    }
    def __init__(self, *args, **kwargs):
    ...
    def setupUtilToolUiBox(self, toolBox):
        ...
        #
        self.simulationManagerButton = guiQtWidgets.QtPressbutton()
...
#
class IfScUtilToolUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    # Utilities Tool
    dicUtils = bscMtdCore.orderedDict()
    ...
    def setupUtilitiesToolPanel(self):
        self.utilsBox = guiQtWidgets.QtToolbox()
...
#
class IfScAnimUploadToolUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    #
    ...
    dicScUpload = {
        ...
        7: 'Action(s)',
        'uploadAnimation': [0, 8, 0, 1, 4, u'Upload ！！！', 'svg_basic/upload']
    }
    #
    dicScExtendUpload = {
        'assemblyComposeUpload': [0, 0, 1, 1, 4, 'Upload / Update Assembly Compose', 'svg_basic/update']
    }
    def __init__(self, *args, **kwargs):
    ...
    def refreshMethod(self):
        if self._connectObject:
            sceneStage = self._connectObject.sceneStage
            #
            self._scAnimUploadButton.setNameString(
                u'Upload {} ！！！'.format(
    ...
    def setupBasicTab(self, layout):
        ...
        #
        self._scAnimUploadUiBox = guiQtWidgets.QtToolbox()
    ...
    def setupExtendTab(self, layout):
        self._scExtendUploadToolUiBox = guiQtWidgets.QtToolbox()
    ...
    def setupTipToolUiBox(self, toolBox):
        ...
        #
        self._scTipUiLabel = guiQtWidgets.QtTextbrower()
    ...
    def setupScUploadUiBox(self, toolBox):
        def setupIndexBranch():
            ...
            #
            self._scWithIndexButton = guiQtWidgets.QtCheckbutton()
            ...
            #
            self._scIndexWithCameraSubButton = guiQtWidgets.QtCheckbutton()
            indexSubLayout.addWidget(self._scIndexWithCameraSubButton)
            self._scIndexWithCameraSubButton.setNameString('Camera')
            self._scIndexWithCameraSubButton.setChecked(True)
            #
            self._scIndexWithAssetSubButton = guiQtWidgets.QtCheckbutton()
            indexSubLayout.addWidget(self._scIndexWithAssetSubButton)
            self._scIndexWithAssetSubButton.setNameString('Asset')
            self._scIndexWithAssetSubButton.setChecked(True)
            #
            self._scIndexWithScenerySubButton = guiQtWidgets.QtCheckbutton()
            indexSubLayout.addWidget(self._scIndexWithScenerySubButton)
            self._scIndexWithScenerySubButton.setNameString('Scenery')
        ...
        def setupPreviewBranch():
            ...
            #
            self._scWithPreviewButton = guiQtWidgets.QtCheckbutton()
            ...
            #
            self._withLightSubButton = guiQtWidgets.QtCheckbutton()
            ...
            #
            self._aviFormatSubButton = guiQtWidgets.QtRadioCheckbutton()
            previewSubLayout.addWidget(self._aviFormatSubButton)
            self._aviFormatSubButton.setNameString('.avi')
            #
            self._movFormatSubButton = guiQtWidgets.QtRadioCheckbutton()
            previewSubLayout.addWidget(self._movFormatSubButton)
            self._movFormatSubButton.setNameString('.mov')
            self._movFormatSubButton.setChecked(True)
            #
            subRefreshCmd()
        #
        def setupCameraBranch():
            self._scWithCameraButton = guiQtWidgets.QtCheckbutton()
        ...
        def setupAssetBranch():
            def subRefreshCmd():
                ...
                for i in checkButtonLis:
                    i.setCheckable(mainCheckButton.isChecked())
            #
            self._scWithAssetButton = guiQtWidgets.QtCheckbutton()
            ...
            #
            self._scAstWithModelCacheSubButton = guiQtWidgets.QtCheckbutton()
            assetSubLayout.addWidget(self._scAstWithModelCacheSubButton)
            self._scAstWithModelCacheSubButton.setNameString('Model Cache')
            self._scAstWithModelCacheSubButton.setChecked(True)
            #
            self._scAstWithSolverCacheSubButton = guiQtWidgets.QtCheckbutton()
            assetSubLayout.addWidget(self._scAstWithSolverCacheSubButton)
            self._scAstWithSolverCacheSubButton.setNameString('Solver Cache')
            self._scAstWithSolverCacheSubButton.setChecked(True)
            #
            self._scAstWithRigExtraCacheSubButton = guiQtWidgets.QtCheckbutton()
            assetSubLayout.addWidget(self._scAstWithRigExtraCacheSubButton)
            self._scAstWithRigExtraCacheSubButton.setNameString('Extra Cache')
        ...
        setupAssetBranch()
        #
        self._scWithSceneryButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self._scAnimUploadButton = guiQtWidgets.QtPressbutton()
    ...
    def _initScRangeConfig(self):
        ...
        self._scRenderValueLabel.setDefaultValue((prsOutputs.Util.rndrImageWidth, prsOutputs.Util.rndrImageHeight))
    #
    ...
    def _scAnimUploadCmd(self):
        ...
        def getPreviewUploadDatum():
            ...
            if mainBoolean is True:
                ...
                cameraData = sceneCameraLis, sceneCameraLis
                # Preview Config
                vedioFormat = [prsOutputs.Util.aviExt, prsOutputs.Util.movExt][self._movFormatSubButton.isChecked()]
...
#
class IfScLightUploadToolUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    #
    VAR_kit__qt_wgt__unit__uiname = 'Scene Light Upload'
    panelWidth = 800
    panelHeight = 800
    #
    ...
    dicScWorkspace = {
        ...
        'setRenderPath': [0, 4, 0, 1, 2, 'Set Path', 'svg_basic/refresh'], 'openRenderPath': [0, 4, 2, 1, 2, 'Open Path', 'svg_basic/fileOpen'],
        # 5
        'updateRenderIndex': [0, 6, 0, 1, 4, 'Update Render Index', 'svg_basic/upload']
    }
    # Deadline
    dicDeadlineOption = {
    ...
    #
    dicDeadlineSubmit = {
        'submitDeadline': [0, 1, 0, 1, 4, 'Submit Deadline', 'svg_basic/upload']
    }
    #
    dicScUpload = {
        'withLight': [0, 0, 0, 1, 1, 'Light File'], 'withRender': [0, 0, 2, 1, 1, 'Render File'], 'withDeadline': [0, 0, 3, 1, 1, 'Deadline'],
        'updateScene': [0, 1, 0, 1, 4, 'Update Scene', 'svg_basic/upload']
    }
    #
    dicMelCommand = {
        ...
        # 2
        'save': [0, 3, 0, 1, 2, 'Save', 'maya/savePreset'], 'load': [0, 3, 2, 1, 2, 'Load', 'svg_basic/load']
    }
    #
    ...
    def setupCustomizeToolUiBox(self, toolBox):
        ...
        #
        self._scRenderPathLabel = guiQtWidgets.QtValueLine()
        toolBox.setInfo(inData, 'renderPath', self._scRenderPathLabel)
        #
        self._scCustomizeLabel = guiQtWidgets.QtValueLine()
    ...
    def setupUploadToolUiBox(self, toolBox):
        ...
        inData = self.dicScUpload
        #
        self.withLightLabel = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.withRenderLabel = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.withDeadlineLabel = guiQtWidgets.QtCheckbutton()
        ...
        #
        self._scLightUpdateButton = guiQtWidgets.QtPressbutton()
    ...
    def setupDdlOptionToolUiBox(self, toolBox):
        ...
        #
        self._ddlJobTimeoutLabel = guiQtWidgets.QtValueArrayLine()
        ...
        #
        self._ddlJobMachineLimitLabel = guiQtWidgets.QtValueArrayLine()
        ...
        #
        self._ddlJobAbortErrorLabel = guiQtWidgets.QtValueArrayLine()
        ...
        #
        self._ddlJobWithEachLayerButton = guiQtWidgets.QtValueLine()
        ...
        #
        self._ddlJobSizePercentLabel = guiQtWidgets.QtValueLine()
    ...
    def setupDdlRenderLayerToolUiBox(self, toolBox):
        self._renderLayerTreeView = guiQtWidgets.QtTreeview()
    ...
    def setupDdlFrameTooUiBox(self, toolBox):
        def setActionData():
            ...
            actionDatumLis = [
                ('Extend Check', ),
                ('Check Odd Frame', 'svg_basic/checkedall', True, checkOddFrameCmd),
                ('Check Even Frame', 'svg_basic/checkedall', True, checkEvenFrameCmd),
                (),
                ('Check Key Frame', 'svg_basic/checkedall', True, checkKeyFrameCmd)
            ]
            #
        ...
        #
        self._frameTreeView = guiQtWidgets.QtTreeview()
    ...
    def setupDdlMelCommandToolUiBox(self, toolBox):
        ...
        def setLoadAction():
            def setActionBranch(timeTag, subOsFile):
                ...
                actionDatumLis.append(
                    (actionExplain, 'svg_basic/file', True, subLoadCmd)
                )
            #
            actionDatumLis = [
                ('Basic', ),
        ...
        #
        self._addToMayaButton = guiQtWidgets.QtCheckbutton()
    ...
    def setupDdlSubmitToolUiBox(self, toolBox):
        ...
        #
        self._ddlSubmitButton = guiQtWidgets.QtPressbutton()
    ...
    def setListRenderLayer(self):
        def setBranch(renderLayer):
            def updateRenderable():
                ...
                #
                treeItem.setIcon('svg_basic/render')
                #
            ...
            treeItem.setNameString([renderLayer, 'masterLayer'][renderLayer == 'defaultRenderLayer'])
            #
    ...
    def _kit__unit__set_build_(self):
        ...
        self._tabWidget.addTab(renderScrollBox, 'Render', 'svg_basic/tab')
        renderScrollBox.setSpacing(2)
        #
        deadlineScrollBox = qtCore.QScrollArea_()
        self._tabWidget.addTab(deadlineScrollBox, 'Deadline', 'svg_basic/tab')
...
#
class IfScAnimManagerUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    #
    VAR_kit__qt_wgt__unit__uiname = 'Scene Animation Manager'
    panelWidth = 800
    panelHeight = 800
    #
    ...
    dicModify = {
        0: 'Variant',
        'variant': [0, 1, 0, 1, 2, 'Variant'], 'setVariant': [1, 1, 2, 1, 2, 'Set Variant', 'svg_basic/switch'],
        2: 'Number',
        'number': [0, 3, 0, 1, 2, 'Number'], 'setNumber': [0, 3, 2, 1, 2, 'Set Number', 'svg_basic/switch'],
        'placeholder': [0, 4, 0, 1, 4, 'Placeholder']
    }
    #
    dicSwitch = {
        'switchToLow': [0, 0, 0, 1, 2, 'Low - Quality [ 0000 ]', 'svg_basic/switch'], 'switchToHigh': [0, 0, 2, 1, 2, 'High - Quality [ 0000 ]', 'svg_basic/switch'],
        #
        'switchToGpu': [0, 2, 0, 1, 2, 'GPU [ 0000 ]', 'svg_basic/switch'], 'switchToRig': [0, 2, 2, 1, 2, 'Rig [ 0000 ]', 'svg_basic/switch'],
    ...
    def setupCheckToolUiBox(self, toolBox):
        ...
        #
        self.reduceAssetNumberButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.reduceAssetNamespaceNamingButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.clearAssetHierarchyButton = guiQtWidgets.QtPressbutton()
    ...
    def setupNamespaceToolUiBox(self, toolBox):
        ...
        #
        self.reduceAssetNamespaceHierarchyButton = guiQtWidgets.QtPressbutton()
    ...
    def setupSwitchToolUiBox(self, toolBox):
        ...
        #
        self.switchToLowButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.switchToHighButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.switchToGpuButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.switchToRigButton = guiQtWidgets.QtPressbutton()
    ...
    def setupAnimExportToolUiBox(self, toolBox):
        ...
        #
        self.rootLabel = guiQtWidgets.QtValueLine()
        toolBox.addInfo('root', self.rootLabel)
        #
        self.animExportTipsLabel = guiQtWidgets.QtTextbrower()
        ...
        #
        self.autoRootButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.exportAnimationButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.importAnimationButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.transferAnimationButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.fromLabel = guiQtWidgets.QtValueLine()
        # toolBox.addInfo('from', self.fromLabel)
        #
        self.toLabel = guiQtWidgets.QtValueLine()
    ...
    def setupRightBottomToolUiBox(self, toolBox):
        ...
        #
        self.namespaceLabel = guiQtWidgets.QtValueLine()
        toolBox.addInfo('namespace', self.namespaceLabel)
        #
        self.fileLabel = guiQtWidgets.QtValueLine()
    ...
    def setListAsset(self):
        ...
        if inData:
            ...
            for seq, (k, v) in enumerate(inData.items()):
                ...
                #
                showName = prsOutputs.Util.assetTreeViewName(assetName, number, assetVariant)
                #
        ...
        if self.modifyToolRefreshEnable:
            ...
            if boolean:
                ...
                if state == 'High - Quality':
                    ...
                    variantLabel.setDatum(prsOutputs.Util.astDefaultVersion)
                    #
    ...
    def setSwtBtnState(self):
        def setBranch(button, explain, statisticalData):
            ...
            button.setNameString('%s [ %s ]' % (explain, str(count).zfill(4)))
        # Enable Switch
        if self.switchToolRefreshEnable:
            ...
            if selectedItems:
                for assetItem in selectedItems:
                    ...
                    if maUtils._isAppExist(tempCfxGroup):
                    ...
                    #
                    if not maUtils._isAppExist(tempCfxGroup):
                        self.disableCfxs.append(assetItem)
                #
    ...
    def setAssetVariant(self):
        ...
        if assetItem:
            ...
            if not newVariant == assetVariant:
                maUtils.setAttrStringDatumForce_(referenceNode, prsOutputs.Util.basicVariantAttrLabel, newVariant)
                assetItem.assetVariant = newVariant
                newShowName = prsOutputs.Util.assetTreeViewName(assetName, number, newVariant)
    ...
    def setAssetNumber(self):
        ...
        if assetItem:
            ...
            if not newNumber == number:
                maUtils.setAttrStringDatumForce_(referenceNode, prsOutputs.Util.basicNumberAttrLabel, str(newNumber).zfill(4))
                newShowName = prsOutputs.Util.assetTreeViewName(assetName, str(newNumber).zfill(4), assetVariant)
                assetItem.setText(0, newShowName)
                maUtils.setMessageWindowShow(
    ...
    def setAssetNumbers(self):
        ...
        if inData:
            isRefresh = False
            attrName = prsOutputs.Util.basicNumberAttrLabel
            maxValue = len(inData)
            for seq, (k, v) in enumerate(inData.items()):
                if self._connectObject:
                    self._connectObject.setProgressValue(seq + 1, maxValue)
    ...
    def getSelRig(self):
        self.selectedAsset = []
        #
        data = maUtils.getSelObjParentFilter(prsOutputs.Util.basicGeometryGroupLabel)
        if data:
            for i in data:
...
#
class IfSimManagerUnit(kitQtWgtAbs.IfToolUnitBasic):
    projectName = currentProjectName
    #
    VAR_kit__qt_wgt__unit__uiname = 'Scene Simulation Manager'
    #
    panelWidth = 800
    panelHeight = 800
    #
    ...
    #
    dicBottomTool = {
        'openLocalFolder': [1, 0, 0, 1, 2, '''Open Local Folder''', 'svg_basic/fileOpen'], 'openServerFolder': [1, 0, 2, 1, 2, '''Open Server Folder''', 'svg_basic/fileOpen'],
        1: 'Cache(s) Check',
    ...
    def setupLeftTabWidget(self, tabWidget):
        ...
        tabWidget.addTab(scrollBox, 'Fur Cache(s)', 'svg_basic/tab')
    ...
    def setupScAstCfxFurCacheWriteToolBox(self, toolBox):
        ...
        #
        self.writeAllNodeButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.continueWriteButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.overrideFrameButton = guiQtWidgets.QtCheckbutton()
        toolBox.setButton(inData, 'overrideFrame', self.overrideFrameButton)
        #
        self._frameValueLabel = guiQtWidgets.QtValueArrayLine()
        ...
        #
        self.overrideSampleButton = guiQtWidgets.QtCheckbutton()
        toolBox.setButton(inData, 'overrideSample', self.overrideSampleButton)
        #
        self._sampleValueLabel = guiQtWidgets.QtValueArrayLine()
        ...
        #
        self._writeScAstCfxFurCacheButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.writeFurCacheTipLabel = guiQtWidgets.QtTextbrower()
    ...
    def setupScAstCfxFurCacheLoadToolBox(self, toolBox):
        ...
        #
        self.readAllNodeButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self.ignoreStaticButton = guiQtWidgets.QtCheckbutton()
        ...
        #
        self._readScAstCfxFurCacheButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.readFurCacheTipLabel = guiQtWidgets.QtTextbrower()
    ...
    def setupRightWidget(self, layout):
        ...
        self._rightTreeView = qtWidgets_.QTreeWidget_()
        layout.addWidget(self._rightTreeView)
        self._rightTreeView.setKeywordFilterWidgetConnect(self._filterEnterLabel)
    #
    ...
    def setupScAstCfxFurCacheModifyToolBox(self, toolBox):
        ...
        self.openCacheLocalFolderButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.openCacheServerFolderButton = guiQtWidgets.QtPressbutton()
        ...
        #
        self.checkAllNodeButton = guiQtWidgets.QtCheckbutton()
        toolBox.setButton(inData, 'checkAllNode', self.checkAllNodeButton)
        #
        self.nurbsHairCacheCheckButton = guiQtWidgets.QtPressbutton()
    ...
    def setListScAstCfxFurNode(self):
        def setSceneBranch(sceneDataDic):
            ...
            def setScAstBranch(assetDataLis):
                def setAstCfxFurBranch(nodeDataLis):
                    def setAstCfxFurSubBranch(furObjectPath):
                        ...
                        furObjectType = maUtils._getNodeShapeCategoryString(furObjectPath)
                        #
                        cacheItemIcon1 = 'svg_basic/local'
                        cacheItemIcon2 = 'svg_basic/server'
                        #
                ...
                for assetIndex, assetCategory, assetName, number, assetVariant, furNodesInAsset in assetDataLis:
                    ...
                    #
                    assetIcon = 'svg_basic/package_object'
            ...
            for (sceneIndex, sceneCategory, sceneName, sceneVariant), assetDataArray in sceneDataDic.items():
                ...
                #
                sceneItemIcon0 = 'svg_basic/package_object'
                sceneIconSubLabel = None
                sceneItemIcon1 = 'svg_basic/time'
                #
        ...
        if nodes:
            for furObjectPath in nodes:
                if furObjectPath in nodeItemDic:
                    furNodeName = maUtils._nodeString2nodename_(furObjectPath)
                    #
