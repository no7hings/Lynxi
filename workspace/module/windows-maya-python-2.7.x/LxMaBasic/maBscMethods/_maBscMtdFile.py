...
class File(maBscMtdCore.Mtd_MaBasic):
    @classmethod
    def exportTo(cls, fileString):
        maBscMtdCore.Mtd_MaFile._maFileExportCommand(fileString)

    @classmethod
    def exportSelectedTo(cls, fileString, objectString, withHistory=False):
        maBscMtdCore.Mtd_MaFile._setMaFileExportSelected(fileString, objectString, withHistory)

    @classmethod
    def exportSelectedWithSetTo(cls, fileString, objectString, setString, withHistory=False):
        maBscMtdCore.Mtd_MaFile._setMaFileExportSelectedWithSet(fileString, objectString, setString, withHistory)

    @classmethod
    def importFrom(cls, fileString, namespace=':'):
        maBscMtdCore.Mtd_MaFile._setMaFileImport(fileString, namespace)

    @classmethod
    def importFromWithGroup(cls, fileString, groupString, namespace=':'):
        maBscMtdCore.Mtd_MaFile._setMaFileImportWithGroup(fileString, groupString, namespace)

    @classmethod
    def importAlembicFrom(cls, fileString, namespace=':'):
        maBscMtdCore.Mtd_MaFile._setMaAlembicImport(fileString, namespace)

    @classmethod
    def referenceFrom(cls, fileString, namespace=':'):
        maBscMtdCore.Mtd_MaFile._setMaFileReference(fileString, namespace)

    @classmethod
    def referenceCacheFrom(cls, fileString, namespace=':'):
        maBscMtdCore.Mtd_MaFile._setMaCacheReference(fileString, namespace)

    @classmethod
    def openFrom(cls, fileString):
        maBscMtdCore.Mtd_MaFile._setMaFileOpen(fileString)

    @classmethod
    def openAsTemporary(cls, fileString):
        maBscMtdCore.Mtd_MaFile._setMaFileOpenAsTemporary(fileString)

    @classmethod
    def openAsBackup(cls, fileString, backupString, timetag=None):
        maBscMtdCore.Mtd_MaFile._setMaFileOpenAsBackup(fileString, backupString, timetag)

    @classmethod
    def saveToServer(cls, fileString):
        maBscMtdCore.Mtd_MaFile._setMaFileSaveToServer(fileString)

    @classmethod
    def saveToLocal(cls, fileString, timetag=None):
        maBscMtdCore.Mtd_MaFile._setMaFileSaveToLocal(fileString, timetag)

    @classmethod
    def updateTo(cls, fileString):
        maBscMtdCore.Mtd_MaFile._setMaFileUpdate(fileString)

    @classmethod
    def new(cls):
        maBscMtdCore.Mtd_MaFile._setMaFileNew()


class AlembicCache(maBscMtdCore.Mtd_MaBasic):
    FileKey = '-file'
